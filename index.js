(() => {
    ...
    const DEFAULT_PRESET = {
        ...
        add: {
            ...
            // 新增：如果数组末尾已经是符合此结构的消息（例如 modify 规则刚把续写消息
            // 改成了 partial:true），就跳过本次插入，避免出现两条连续的 assistant 消息。
            skipIfEndMatches: {
                partial: true,
            },
            maxAdds: 1,
    ...
    function normalizePreset(p) {
        ...
        return {
            ...
            add: {
                ...
                skipIfEndMatches: a.skipIfEndMatches && typeof a.skipIfEndMatches === 'object' ? clone(a.skipIfEndMatches) : (d.add.skipIfEndMatches ? clone(d.add.skipIfEndMatches) : null),
    ...
    function insertAddRule(data, preset) {
        ...
        if (add.skipIfEndMatches && arr.length && deepMatch(arr[arr.length - 1], add.skipIfEndMatches)) {
            return { added: 0, reason: '数组末尾已存在匹配结构，跳过新增（避免出现连续的 assistant/partial 消息）' };
        }
    ...
    function buildUi() {
        ...
        <div class="inline-drawer">
          ...
          <div class="inline-drawer-content">
            ...
            <details class="jtx-section"><summary>插入</summary>
              ...
              <label>跳过条件（数组末尾若匹配此结构则不新增，可空）</label><textarea id="jtx-add-skip" rows="3" placeholder='例如 {"partial":true}'></textarea>
            ...
            <details class="jtx-section"><summary>高级 / 提示</summary>
              ...
              <div class="jtx-hint">当前默认预设：模型关键词仅 Kimi。查找 reasoning_content 为任意值（通配符 *）的 assistant partial 消息并整体替换为 reasoning_content = &lt;thinking&gt;（应对"续写"场景，酒馆自动补的思考文字每次都不一样，不能写死匹配）。插入默认开启：若消息数组末尾还不是 partial 消息（正常发起新一轮时），会在末尾追加 reasoning_content = &lt;thinking&gt; 的 assistant partial 消息；若末尾已经是 partial 消息（说明上面的篡改规则刚处理过，或已存在同类消息），会自动跳过插入，避免出现两条连续的 assistant 消息导致请求非法。</div>
            </details>
        ...
        function fill() {
            ...
            $('#jtx-add-skip').value = p.add.skipIfEndMatches ? dump(p.add.skipIfEndMatches) : '';
        ...
        function readIntoPreset() {
            ...
            p.add.skipIfEndMatches = $('#jtx-add-skip').value.trim() ? parseField('#jtx-add-skip') : null;
